...
\rightheader{Gaing at the Robot's Head Reflects our Tendency to Anthropomorphise}   % This should be the title, or a shortened version of the title if your title is long.
...
given to the robot effectively influenced how anthropomorphic the participants
perceived the robot.

We interpret this result as the participants building in each condition a different set of
expected cognitive skills for the robot: when a participant hears the robot
being instructed ``Pick up your favourite toy'', she implicitly (and likely,
unconsciously) supposes that the robot has the cognitive capabilities required
to \emph{prefer} one toy to another one, which would typically implies that the robot
understand the affective dimensions of the different toys. By performing this
reasoning, we believe the human (likely unconsciously) does attribute cognitive and emotional
traits to the robot that are akin to humans, and as a result, anthropomorphise
more the robot.
