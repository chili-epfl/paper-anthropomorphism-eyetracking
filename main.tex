...
\rightheader{Gazing at a Robot's Head Reflects our Tendency to Anthropomorphise}   % This should be the title, or a shortened version of the title if your title is long.
...
\title{Gazing at a Robot's Head Reflects our Tendency to Anthropomorphise}
...
machine. Besides, by comparing this gaze behaviour to a human baseline, we further
...
significantly different gaze behaviours in two experimental conditions that
...
\begin{itemize}
    \item people who generally tend to anthropomorphise exhibit gazing
        ...
        gaze behaviour towards agents is impacted by our tendency to
        anthropomorphise.
...
To compare gaze behaviours across the robot and human conditions, we besides
...
task). We found a significant mean difference ($M_{\text{machine-like}}=-0.46
(SD=1.21)$
\vs $M_{\text{human-like}}=1.16 (SD=1.00)$) in this
manipulation check ($F(1,55) = 30.73, p < .001$): the participants who watched
...
human-like tasks ($\Delta_{\text{machine-like}}=2.97 (SD=9.60)$
\vs $\Delta_{\text{human-like}}=9.94 (SD=9.93)$, $F(1,55) = 7.27, p = .009$. In other words, after being exposed to short videos of a robot
...
\deltagaze and
the initial tendency to anthropomorphise \anti{} (Pearson Correlation
Coefficient $r(55) = -0.38,
p = .003$, figure~\ref{h2}). The higher the (initial) tendency to anthropomorphise, the smaller
the difference in gaze behaviours between the robot and human conditions. This
...
($F(1,55) = 4.07, p = .04$\fixme{put the exact p value here}). For all other AOIs groups, the fixations across
...
gaze behaviour similarity, we have evidenced that people with an higher
...
correlation with gaze behaviour: the more you tend to anthropomorphise, the
...
and we did not anticipate any transfer of the anthropomorphic traits ascribed
onto the robot seen in the videos, to the broader tendency to anthropomorphise
robots in general, as assessed in the post-questionnaire. Our result challenge
this assumption: \deltaant is significantly higher for the participants who have
been primed with higher cognitive expectations before watching the robot
performing the tasks. This suggests a form of transfer: participants changed
their minds about how human-like robots are from their perception of one
specific interaction situation with a specific robot. We did not test
how long this effect lasts (we suspect it might quickly decrease).
